...
#Updates: Updated on 4/15/25 by Ray Danner to remove data from smoothing lines for the Ellwood Main weather station in mid-March (when the weather station was not working properly)
...
end_date <- "2025-04-15" # Example end date
...
# Subset data to remove gap of time when weather station was not functioning properly
subset_main_early <- ellwood_main_data |> filter(local_time <= as.POSIXct("2025-03-15 16:15:00"))

subset_main_late <- ellwood_main_data |> filter(local_time >= as.POSIXct("2025-04-01 22:18:00"))

# Create the plot
ggplot(ellwood_main_data, aes(x = local_time, y = speed, color = measure)) +
    geom_point(alpha = 0.1, size = 0.5) +
    geom_smooth(data=subset_main_early, method = "gam", se = TRUE, alpha = 0.2) +
    geom_smooth(data=subset_main_late, method = "gam", se = TRUE, alpha = 0.2) +
    ...
    caption = "*The gap in the graph was caused due to a malfunction in the weather station"
) +
theme(
    plot.caption = element_text(size = 10, hjust = 1, face = "italic", color = "gray30")
)+
...
combined_wind_data <- df %>%
   ...
   # Prepare data from Ellwood Main
subset_main_early$station <- "Ellwood Main"
subset_main_late$station <- "Ellwood Main"
...
ggplot(combined_wind_data, aes(x = local_time, y = speed, color = station)) +
    ...
    caption = "*The gap in the graph was caused due to a malfunction in the Ellwood Main weather station"
) +
theme(
    plot.caption = element_text(size = 10, hjust = 1, face = "italic", color = "gray30")
)+
